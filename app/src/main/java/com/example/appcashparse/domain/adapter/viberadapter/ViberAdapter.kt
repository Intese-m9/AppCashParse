package com.example.appcashparse.domain.adapter.viberadapter
...
class ViberAdapter: RecyclerView.Adapter<ViberAdapter.StartViewHolder>() {
    //устанавливаем список
    var listViber = emptyList<FileCash>()
    class StartViewHolder(view: View): RecyclerView.ViewHolder(view)
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ViberAdapter.StartViewHolder {
        val view = LayoutInflater.from(parent.context).inflate(R.layout.item_listcash,parent,false)
        return StartViewHolder(view)
    }

    override fun onBindViewHolder(holder: ViberAdapter.StartViewHolder, position: Int) {
